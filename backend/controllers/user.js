...
exports.verifyEmail = async (req, res) => {
    ...
    if (!token) return sendError(res, "Please Submit a valid OTP!");

    const isMatched = await token.compareToken(OTP)
    if (!isMatched) return sendError(res, "Please sumbit the correct OTP!");
...
exports.resendEmailVerificationToken = async (req, res) => {
  ...
  if (alreadyHasToken) return sendError(res, "only after ten minutes you can request for another token!");
...
exports.forgetPassword = async (req, res) => {
  ...
  const resetPasswordUrl = `http://localhost:3000/auth/reset-password?
